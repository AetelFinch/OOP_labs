...
public:
    ...
    bool erase(const K& k);  // Deletes an element by the specified key

    bool insert(const K& k, const V& v);  // Inserting into the container. The return value is the success of the insertion

    bool contains(const K& k) const;  // Checking for the presence of a value for a given key
...
private:
    std::vector<std::list<std::pair<K, V>>> *_table; //vector of lists of pairs of Keys and Values
    size_t _table_size;
